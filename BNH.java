...
public class BNH {
	 ...
	 * 
	 * @param node Binomial tree
	 */
	private BNH(HeapNode node) {
		this.min = node;
		this.first = node;
		this.last = node;
	...
	/**
	 * Fixes the heap's properties: size, treesNum, min, last. This method should be
	 ...
	 * 
	 * @param heap1 First heap to merge
	 * @param heap2 Second heap to merge
	 * @return Merged heap
	 */
	public static BNH merge(BNH heap1, BNH heap2) {
		BNH h = new BNH();
	 ...
	 * 
	 * NOTE: The heap is not necessarily valid after calling addTree. see merge for
	 * details.
	 * 
	 ...
	 * 
	 * Return an array containing the sizes of the trees that represent the heap in
	 * ascending order.
	 * 
	 * Time complexity: O(logn)
	 */
	...
	public class HeapNode {
		 ...
		 * 
		 * @param next Pointer to new following sibling node
		 */
		 ...
		 * 
		 * @param prev Pointer to new previous sibling node
		 */
		 ...
		 * 
		 * @param leftmostChild Pointer to new leftmost child node
		 */
		 ...
		 * 
		 * @param rightmostChild Pointer to new rightmost child node
		 */
		...
		/**
		 * Returns the actual size and rank of the node, independently of the other
		 * method's validity.
		 * 
		 * May return null if the node was found invalid, i.e.: its root value is not
		 * the minimal value, one of its children is an invalid heap node, or its size
		 * isn't 2^rank.
		 * 
		 * @return An object containing the collected actual size and rank of the node,
		 *         null if the node was found invalid.
		 */
		 ...
		 * 
		 * precondition: other != null precondition: other.getValue() > this.getValue()
		 * precondition: other.getRank() == this.getRank() postcondition: this.getRank()
		 * = 1 + $prev(this.getRank()) postcondition: this.getRightmostChild() == other
		 */
